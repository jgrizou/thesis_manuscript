...
There is a multitude of weak points in this method and we note that modeling a distribution of probability using a normal distribution violates the normality assumption. Using a beta distribution would have been more suitable but we could not find an anlytical solution to the difference between two beta distributions. However, we tried to use more standard statistical test, such as the one tailed Student's t-test or Welch's t-test, but the results were not satisfying as those tests only check whether or not the means of the distributions are equals.
...
As there is an infinity of possible goal state, the agent can not estimate the probability of all possible task in parallel. We will rely on a particle filter based apporach \cite{gordon1993novel,doucet2009tutorial,thrun2002particle}. The main idea consist of sampling a finite number of task and compute a confidence measure for each of those task. Given the ranking between them, we will aplly a resample step which consist of keeping some of the best task and sample a bunch of new ones.
