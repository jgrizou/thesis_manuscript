...
\begin{figure}[!ht]
\centering
\includegraphics[width=\plotsize\columnwidth]{\visualspdf/worlds_and_datasets/finger_signals_color.pdf}
\caption{N}
...
We selected an other option which rely on sampling different classifier from a meta-classifier, which allow us to generate classifiers at a low computational cost. Then given many classifiers for each task, we will compare the likelihood predicted by those classifiers and rank the task by a statistical test on the classifier evaluation. We describe each step of this process in the following paragraph.
 ...
 which consist of sampling different classifiers
With this method we only have to train one ``meta'' model for each hypothesis, which simply encodes a probability distribution over the mean and covariance. 

The lower the number of point the more the distribution will be spread , the more, the less variability in the generated classifiers.

directly use our to output the posterior probability of a signal given a class, which was used to estimate the probability of that point to belong to each of the class. By 

Given this model we can sample, for very low computational cost, a multitude of possible QDA classifiers by sampling a mean and covariance for each class.

\subsection{Hypothesis selection and generation methods}
...
There is many parameters that will influence the performance of such an algorithm. We can change the number of task sampled, the number of task and the criteria of the task that stay in the pool from step to step, and we can change the method used to sample new task. 
...
Our random generation of task simply keep the best hypothesis and generate 49 new task hypothesis randomly.

Our active task generation method
which simply selects new task around the current best task hypothesis
...
\todo{maybe a video of the evolution of things, or just some snapshots here}
