...
Still, this last approach is not taking into account the intrinsic quality of the classifier itself before taking into account its prediction. By this we ask the question of knowing how reliable the prediction of the classifier are? A common method to evaluate the uncertainty on the classifier prediction is to use a cross-validation procedure \todo{refer to intro part} to estimate the confusion matrix associated to the classifier. Such confusion matrix allow to infer the conditional probability of one label given the label prediction from the classifier $p(l = l_k| l^c = l_q, \theta)$, for a every combination of $k$ and $q$ in $1, \ldots, L$. Which can be used to temperate the predictions of the classifier. 
...
\todo{describe the conditional estimation from the joints}
