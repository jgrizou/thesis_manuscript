...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=\tworldsize\columnwidth]{\imgpath/Tworld_with_state_number_and_action.png}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.5\columnwidth]{\imgpath/feedback_signals_color.png}
  \caption{The feedback signals used by our teacher in our visual examples. A signal of meaning ``correct'' will be at the right side of the feature space, and a signal of meaning ``incorrect'' will be on the left side. Importantly, the agent will never have access to the label information.}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=\tworldsize\columnwidth]{\imgpath/Tworld_feedback_unlabeled_one_action.png}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=\tworldsize\columnwidth]{\imgpath/Tworld_feedback_unlabeled.png}
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[b]{\tworldsize\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{\imgpath/Tworld_feedback_unlabeled_one_action.png}
        \caption{Feedback signal as received by the agent without label.}
    \end{subfigure}\\
    \begin{subfigure}[b]{\tworldsize\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{\imgpath/Tworld_feedback_labeled_G1.png}
        \caption{Feedback signal labeled according to G1.}
        \label{fig:TworldLabelG1}
    \end{subfigure}
    \begin{subfigure}[b]{\tworldsize\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{\imgpath/Tworld_feedback_labeled_G2.png}
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[b]{\tworldsize\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{\imgpath/Tworld_feedback_unlabeled.png}
        \caption{Feedback signal as received by the agent without label.}
    \end{subfigure}\\
    \begin{subfigure}[b]{\columnwidth}
        \centering
        \includegraphics[width=\columnwidth]{\imgpath/Tworld_feedback_labeled_all_actions.png}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.5\columnwidth]{\imgpath/guidance_4_signals_color.png}
  \caption{The guidance signals used by our teacher in our visual examples.}
...
\item If the true intended meaning associated to each user signal was known it would be possible to train a classifier with reasonable accuracy. We will later see that the performance of the system are highly impacted by the quality of the training data. \todo{ref to where we we will see this}
...
We will see from results in chapter \todo{ref to where we we will see this} that our algorithm can cope with highly overlapping data where the classifier produce close to random prediction. In such cases the algorithm will not make decision on which task is the correct one.
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=\tworldsize\columnwidth]{\imgpath/lineworld_with_state_number_and_2action.png}
  \caption{The line world and the available actions.}
  \label{fig:lineworld}
\end{figure}

The interaction follows the same protocol as in previous examples, and following the feedback frame. As depicted in Figure~\ref{fig:lineworldfeedback2action}, after several interaction steps, the interpretation hypothesis for G1 and G2 display symmetric properties.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\tworldsize\columnwidth]{\imgpath/feedback_2actions.png}
  \caption{}
  \label{fig:lineworldfeedback2action}
\end{figure}


We define two task hypothesis as being symmetric if it is theoretically impossible to differentiate between those two. Therefore we will not considered environments holding this symmetric properties. One way to solve this problem is to add a ``goal reached'' action that is valid only at the goal state. 
...
\todo{examplify the resulting stuff}
...
\todo{showcase the guidance frame}
...
\todo{example with gaussian fitting as for AAAi, do it again with the new visuals}

\todo{we capture by the assumption of the classifier on the spacial organization of data, for example the gaussina classifier assume data are distrivuted as gauusian stuff...
alos mot of the algorithm regularize their cost function to avoid overfitting problem and indeed tries to identify the underlying structure of the data, i.e. a way to separte classes with the less complex model}
...
\todo{To sum up, the same process is active in both phase: non-expected prediction that impact directly on the likelihood, and a decrease of the confidence on the classifier prediction. In phase one, all hypothesis model there own stuff, there is less non-expected prediction, but the more we interact the less we trust some classifier which mixes labels. In phase two, all hypothesis start with the same classifier, there is more non-expected prediction, and the more we interact the less we trust some classifier which mixes the labels (but here the time to radically change the trust we put in one classifier is long as they all start with the same data, the more we have similar data the less this plays a role.)}
...
Note that all three dataset have accuracy close to 100 percent on the speech dataset defined above. \todo{provide numbers}. This obviously a quite optimal scenario, we will see in next chapter \todo{which part} how our algorithm behave with data of poorer quality.
...
\todo{explain that we still compares with he frames labels using our update equations}
