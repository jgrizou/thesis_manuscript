...
Measuring task uncertainty follows along the same lines as previous work \cite{macl09airl}. The idea is to take a query-by-committee approach where each ``member'' of the committee, i.e. each task hypothesis $\xi_k$, influences the process according to its importance in the committee, i.e its respective likelihood $\L^{\xi_k}$. We define a vector that accumulates the weighted optimal actions of each hypothesis: 
%
\[
c(s,a) = \sum_i \L^{\xi_i} \delta(\pi(s,a)^{\xi_i}>0)
\]
%
where $\delta$ is a Dirac function that is 1 if the argument is true and zero otherwise. For each state, the vector entropy of the $c(s,a)$ measures the disagreement between hypotheses. We can make a state-wide measure of uncertainty by summing the entropies corresponding to each state-action pair:
%
\[
v_t(s)=\sum_a \mathcal{H}(c(s,a))
\]
