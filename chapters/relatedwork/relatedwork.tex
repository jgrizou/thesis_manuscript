...
Thus, calibration free methods are an important step to apply this technology in real applications \cite{millan10}. But despite the importance of calibration-free BCI, there are few BCI applications that are able to calibrate themselves during operation.

Some work considers online adaption of a classifiers. In invasive BCI, Orsborn et al. proposed a method to learn from scratch and in closed loop a decoder for known targets using pre-defined policies to each target \cite{Orsborn2012}
. However, the approach requires to provide a seed to the decoder, i.e. to start with a prior, and a warm-up period of around 15 minutes. In \cite{vidaurre2010towards} the authors show that it is possible to adapt the decoder online for long term operation using sensory-motor rhythms. Similarly in \cite{schettini2014self}, even if the authors untitled their work as a self-calibration method, it is actually continuous adaptation of the classifier. They perform a first short calibration procedure which is used as a prior for later runs where the system re-calibrate itself continuously.

\todo{add one or two online adaptive classifiers BCI}

While the above methods allow a more flexible and online adaptation to each user specificity, they are not strictly calibration-free methods. Indeed, they require a relative good prior on the decoder of brain signals beforehand.

To our knowledge, only one group of researchers achieved true calibration-free interaction using BCIs technologies \cite{kindermans2014true}. We detail their work in the following subsection.
